...
struct TRAIN {
	...
	uint wagons;
	const char* station;
} trains[] = { {"Choo Choo", 5u, "Chicago"}, {"Jimmy", 3u, "Chicago"}, {"Jackie", 1u, "Atlanta"}, {"Maureen", 2u, "Louisville"}, {"Johnny", 58u, "Cleveland"}, {"Alex", 6u, "Cleveland"}, {"Connie", 4u, "Cleveland"}, {"Tara", 7u, "Cleveland"}, {"Andy", 2u, "Toronto"}, {"Charlie", 6u, "Buffalo"}, {"Lukas", 2u, "Atlanta"}, {"Derrick", 10u, "Louisville"}, {"Rachael", 4u, "Detroit"}, {"Harry", 5u, "Detroit"} };
...
protected:
	...
	void run_part3() {
		...
		sim.departNextTrain("South Bend", "Detroit"); // should be Johnny again
		sim.step();
		...
		sim.removeTrain("Johnny"); // train scheduled for removal
		UNIT_TEST_TRUE(sim.trainExists("Johnny"), "Johnny cannot be removed while traveling");
		sim.step();
		UNIT_TEST_EQUAL(sim.distanceTraveled("Johnny"), 2u, "Another step for Johnny");
		sim.step();
		UNIT_TEST_EQUAL(sim.distanceTraveled("Johnny"), 3u, "Yet another step for Johnny");
		sim.step();
		UNIT_TEST_EQUAL(sim.distanceTraveled("Johnny"), 4u, "One more step for Johnny");
		sim.step();
		UNIT_TEST_TRUE(!sim.trainExists("Johnny"), "Johnny arrived at station and is now removed");

		sim.addTrain("Johnny", 33u, "Cleveland");
		UNIT_TEST_TRUE(sim.trainExists("Johnny"), "But Johnny is back");
		UNIT_TEST_EQUAL(sim.whereStationed("Johnny"), string("Cleveland"), "Johnny is back in its native Cleveland");
		...
		// wagons cannot be removed unless at station
		sim.step();
		sim.removeWagon("Rachael"); // flag for wagon removal upon arrival at station
		sim.print();
		cout << endl;
		sim.step();

		UNIT_TEST_EQUAL(sim.whereStationed("Rachael"), string("Cleveland"), "Rachael is in Cleveland");
		UNIT_TEST_EQUAL(sim.numberWagons("Rachael"), 3u, "Rachael arrives and wagon is removed immediately upon arrival");
